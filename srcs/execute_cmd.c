...
/*   Updated: 2023/06/15 13:48:28 by pfuentes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	search_cmd(t_mshell *mshell, t_list *tokens)
{
	...
	printf("Hacer matriz de cmd\n");
	cmd = cmd_matrix(tokens);
	cont = 0;
	printf("Matriz del comando: \n");
	while (cmd[cont])
	{
		printf("%s\n", cmd[cont]);
		cont++;
...
void	child_execute(char **arg, t_mshell *mshell)
{
	...
	if (dup2(mshell->data.output, STDOUT_FILENO) < 0)
		printf("No redireccionó bien\n");
